...
{	
	...
	shmid = shm_open(SHARED_MEM_NAME, O_CREAT | O_RDWR, 0666);
	if (shmid == -1) {
		perror("shm_open");
		exit(1);
	}
	...
	if (ftruncate(shmid, fileSize) == -1) {
		perror("ftruncate");
		exit(1);
	}

	// Map the shared memory object into the address space of the process
	sharedMemPtr = mmap(NULL, fileSize, PROT_READ | PROT_WRITE, MAP_SHARED, shmid, 0);
	...
	bytesRead = read(fd, sharedMemPtr, fileSize);
