...
void recvFile(int signNum)
{
    ...
    int shmid = shm_open(SHARED_MEM_NAME, O_RDONLY, 0666);
    if (shmid == -1) {
        perror("shm_open");
        exit(1);
    }
    ...
    // Write shared memory contents to the file
    bytesWritten = write(fd, sharedMemPtr, memSize);
    if (bytesWritten < 0) {
        perror("write");
        exit(1);
    }
...
{	
	...
	// handler
	if (signal(SIGUSR1, recvFile) == SIG_ERR) {
        perror("signal");
        return 1;
    }
